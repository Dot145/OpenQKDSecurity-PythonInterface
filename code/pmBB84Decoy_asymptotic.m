...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmBB84Decoy_asymptotic(decoys, pz)
    [protocolDescription,channelModel,leakageEC]=setDescription();
    parameters=setParameters(decoys, pz);
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('pmBB84ReducedDecoyChannel'); %contains asymptotic decoy state analysis in file
...
function parameters=setParameters(decoys, pz)

    parameters.names = ["misalignment","loss", "etad","pz","pd","decoys", "f", 'fullstat', 'time', 'ext']; %BB84 Decoy
...
function solverOptions=setOptions()
    ...
    solverOptions.solver1.maxiter = 10;%10;

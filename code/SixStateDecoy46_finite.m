...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = SixStateDecoy46_finite(decoys, pzA, pzB, pxB)
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('FiniteSixStateDecoyChannel'); %contains asymptotic decoy state analysis in file
...
function parameters=setParameters(decoys, pzA, pzB, pxB)
    ...
    parameters.names = ["misalignment","loss", "etad", "depol","pzB","pzA", "pxB","pd","decoys", "f", ...
        'fullstat', 'time', 'ext', "N","ptest","eps","alphabet","postselection","physDimAB", "decoyprobs"]; %BB84 Decoy
...
%     parameters.scan.loss = 1-10.^(-1*linspace(0,3,21));%1-10.^(-1*linspace(1.6,2,11));%linspace(0, 0.99, 21);%
%     parameters.scan.N = [1e9, 1e12, 1e15, 1e18];
%     parameters.scan.misalignment = linspace(0,pi/6,20);

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    
    parameters.fixed.misalignment = 0;%0.01; 
    parameters.fixed.depol = 0.0;
    ...
    % px = 0.5, pz = 0.25
    parameters.fixed.pzB = 2/3;%0.25;%pzB;%
    parameters.fixed.pxB = 1/6;%0.5;%pxB;%
    parameters.fixed.pd = 0;%6e-7;%pd;%1e-6; %dark count probability
    parameters.fixed.f = 1.16;
    parameters.fixed.fullstat = 1;
    parameters.fixed.loss = 0;%0.7008;%loss;%0.998;
    parameters.fixed.etad = 1;
%     parameters.fixed.time = 1;
    parameters.fixed.decoys = [0.85, .1, 0]; %first is signal, other two are decoys
    parameters.fixed.decoyprobs = [1/3,1/3,1/3];%[0.95, 0.04, 0.01];
...
%     parameters.optimize.mu1 = [0.0001,0.1,.5]; %can optionally optimize laser intensity (need to comment it out mu1 in 'fixed' category)
